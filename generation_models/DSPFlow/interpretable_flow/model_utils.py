...
class AdaLayerNorm(nn.Module):
    def __init__(self, n_embd, proto_dim):
        ...
        # ---- prototype embedding -> FiLM ----
        self.proto_mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(proto_dim, n_embd * 2),
        )

        # ---- layer norm ----
        self.layernorm = nn.LayerNorm(n_embd, elementwise_affine=False)

        # ---- VERY IMPORTANT: zero init prototype branch ----
        nn.init.zeros_(self.time_mlp[-1].weight)
        nn.init.zeros_(self.time_mlp[-1].bias)

        nn.init.zeros_(self.proto_mlp[-1].weight)
        nn.init.zeros_(self.proto_mlp[-1].bias)
